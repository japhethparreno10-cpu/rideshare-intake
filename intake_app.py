...
# =========================
# WAGSTAFF QUESTION FLOW (YOUR 1–105)
# =========================
def render_wagstaff_questions():
    ...
    with st.form("wagstaff_form", clear_on_submit=False):
        ...
        # 5. Client Name (First/Middle/Last with given sample)
        ccols = st.columns([1,1,1])
        with ccols[0]:
            c_first = st.text_input("First Name", value="")
        with ccols[1]:
            c_middle = st.text_input("Middle Name", value="")
        with ccols[2]:
            c_last = st.text_input("Last Name", value="")
        ...
        affirm = st.radio("103. Do you affirm the information is true and correct (including whether you've signed with another firm)?", ["Yes","No"], horizontal=True)
    ...
    with colB:
        # Export CSV of answers + intake, if available
    ...
    with colC:
        st.caption("All 1–105 Wagstaff questions implemented. Tell me any wording tweaks or fields to be required.")

# =========================
# TRITEN PLACEHOLDER (still here; send exact list to wire in)
# =========================
